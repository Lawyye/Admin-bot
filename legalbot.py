...
c = conn.cursor()
c.execute("CREATE TABLE IF NOT EXISTS requests (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, name TEXT, phone TEXT, message TEXT, created_at TEXT, status TEXT DEFAULT 'new')")
conn.commit()
...
async def save_request(message: types.Message, state: FSMContext):
    ...
    with conn:
        conn.execute("INSERT INTO requests (user_id, name, phone, message, created_at, status) VALUES (?, ?, ?, ?, ?, ?)",
...
async def get_requests(request: Request):
    ...
    return [{"id": r[0], "user_id": r[1], "name": r[2], "phone": r[3], "message": r[4], "created_at": r[5], "status": r[6]} for r in rows]
...
async def admin_html(request: Request):
    ...
    async function load() {{
        ...
        document.body.innerHTML += '<ul>' + data.map(r => `
            <li>
